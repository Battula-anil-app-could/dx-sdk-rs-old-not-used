	...
	let gen = match &ast.data {
		syn::Data::Struct(data_struct) => {
			...
			quote! {
				impl #impl_generics dharitri_codec::TopDecode for #name #ty_generics #where_clause {
					fn top_decode<I: dharitri_codec::TopDecodeInput>(top_input: I) -> Result<Self, dharitri_codec::DecodeError> {
						let bytes = top_input.into_boxed_slice_u8();
						let input = &mut &*bytes;
						let result = #name #field_dep_decode_snippets ;
						if !input.is_empty() {
							return Result::Err(dharitri_codec::DecodeError::INPUT_TOO_LONG);
						}
						Ok(result)
					}
		...
		syn::Data::Enum(data_enum) => {
			...
			if is_fieldless_enum(&data_enum) {
				...
				quote! {
					impl #impl_generics dharitri_codec::TopDecode for #name #ty_generics #where_clause {
						fn top_decode<I: dharitri_codec::TopDecodeInput>(input: I) -> Result<Self, dharitri_codec::DecodeError> {
							match <u8 as dharitri_codec::TopDecode>::top_decode(input)? {
								#(#top_decode_arms)*
								_ => Result::Err(dharitri_codec::DecodeError::INVALID_VALUE),
							}
						}
